...
def update_train(model, data_loader, optimizer, epoch, device, config, iteration, lr_schedule, older_params=None, importance=None, w=None):
    ...
    for i, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # store current model 
        curr_feat_ext = {n: p.clone().detach() for n, p in model.named_parameters() if p.requires_grad}
        ...
        unreg_grads = {n: p.grad.clone().detach() for n, p in model.named_parameters()
...
def main_SI(args,config,industry_id_label, all_id_info, device): 
    ...
    w = {n: torch.zeros(p.shape).to(device) for n, p in model.named_parameters() if p.requires_grad}
    older_params = {n: p.clone().detach() for n, p in model.named_parameters()
                if p.requires_grad}
   ...
   #####task_list########
    ...
    for iteration , task_i in enumerate(config['task']):
        ...
        with torch.no_grad():
            damping = 0.1
            curr_params = {n: p for n, p in model.named_parameters() if p.requires_grad}
            for n, p in importance.items():
                importance[n] += w[n] / ((curr_params[n] - older_params[n]) ** 2 + damping)
                w[n].zero_() 

        # Store current parameters for the next task
        older_params = {n: p.clone().detach() for n, p in model.named_parameters() if p.requires_grad}
